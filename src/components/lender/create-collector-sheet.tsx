...
interface CreateCollectorSheetProps {
	isOpen: boolean;
	onClose: () => void;
}

export function CreateCollectorSheet({
	isOpen,
	onClose,
}: CreateCollectorSheetProps) {
	const createCollector = useCollectorsStore((s) => s.createCollector);
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (result.success) {
			...
			setErrorMsg(result.error ?? "Error al crear cobrador");
		}
	};

	return (
		<BottomSheet isOpen={isOpen} onClose={handleClose}>
			<h2 className="text-lg font-bold mb-1">Crear Cobrador</h2>
			<p className="text-text-muted text-sm mb-6">
				Ingrese los detalles para crear un nuevo cobrador en el sistema.
			</p>
