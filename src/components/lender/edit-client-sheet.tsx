...
export function EditClientSheet({
	...
	useEffect(() => {
		if (client) {
			...
			setSelectedRoute(client.route ?? null);
			setNewRouteInput("");
			setErrorMsg(null);
		}
	}, [client]);
	...
	return (
		<BottomSheet isOpen={isOpen} onClose={handleClose}>
			<h2 className="text-lg font-bold mb-1">Editar Cliente</h2>
			<p className="text-text-muted text-sm mb-6">
				Modifica los datos de {client?.full_name}.
			</p>
			...
			<form onSubmit={handleSubmit} className="flex flex-col gap-5">
				...
				<button
					type="submit"
					disabled={updateClient.isPending || createRoute.isPending}
				...
				>
					{updateClient.isPending || createRoute.isPending ? (
